...
- `[~]` Đã port board/solution/region guide cố định và contract layout width 919; presenter chưa dựng board tutorial.
- `[~]` Đã port state machine đủ 7 bước: đặt mèo, xác nhận vùng, đánh dấu hàng/cột, hàng xóm và free play; còn nối view/input.
...
- `[ ]` Port hand loop, select frame, mask/mirror cells, message/submessage và fireworks.
- `[~]` Đã port domain default/check/IQ theo `GuideFeedbackConfig`; SuccessCheck/IQ bar/fireworks presenter còn thiếu.
- `[~]` Đã port `TutorialDiagonalConfig`, `GuideFeedbackConfig` và dùng `DoubleTapProtectConfig` mặc định từ source; chờ test runner.
- `[~]` Đã có committer idempotent lưu `tutorial_done`; route Game level 1 và đóng Tutorial sẽ nối khi presenter/page tồn tại.
- `[~]` State reset/recreate quay về step 1 đúng nguồn và có fixture; app lifecycle PlayMode còn chờ.
...
- `[ ]` Port Home layout, animation, level/progress, Start, Settings và entry slots.
- `[ ]` Port Settings: music/sound/vibration/people/language và back.
- `[ ]` Port localization keys, locale selection, font fallback và refresh text động.
- `[ ]` Port How-to-play hai biến thể cần thiết theo default config.
- `[ ]` Port Bank page/model, browser level thường/LK/LK Modified và launch params.
