...
| UI stack/registry/popup queue | `[~]` | Registry asset hiện có 25 page, AppScene/bootstrap và handler chính đã nối. PlayMode đạt startup/navigation, release-frame guard, local timed blocker, 96 vòng abort-close/reopen có Z compact, mask ổn định và concurrent one-flight. Còn nhánh phụ/touch thiết bị/pixel parity |
| Tutorial/Home/Settings/Bank/Result | `[~]` | Đã có page/prefab/registry và runtime route chính; Main Result, Bank SP result và rewarded revive đã qua AppScene PlayMode. Còn win-toast config, các Bank pool khác, Daily/Meta và visual parity |
...
| Automated Unity tests | `[~]` | Unity Test Runner thật chạy tự động qua hai Editor-only bridge: EditMode **511 passed, 0 failed**; PlayMode AppScene/navigation/Settings/Main/Bank/rewarded revive/transition-input stress **9 passed, 0 failed**. Còn app lifecycle và device suite |
...
- `[~]` Đã port page cache, stack, z-step, fullscreen occlusion, mask ref-count, back request, release-frame guard, source `block_input_briefly` và one-flight prewarm; `AppScene` đã nối shared overlays bằng serialized reference. PlayMode dùng pointer down/up thật để khóa release frame, refresh/cleanup local blocker, 96 vòng close/reopen không duplicate event/kẹt `Closing`/rò mask, ép Z compact và hai `ShowAsync` đồng thời chỉ tạo/show một page. Còn touch thiết bị và pixel parity.
...
- `[~]` Đã port contract và `HomePagePresenter`: level/hard state, ba A/B config mặc định, Start/Settings/Profile/Back handlers, safe-top header, flow shader, source animation marker/timing, cleanup và level text động. `daily_streak/leaderboard_func/hard_button` nay dùng chung `HomeConfigSet` trong `AbConfigRuntime`; AppScene startup/reopen, `StartBtn`→Game, `SettingsBtn`→Settings và các entry meta đạt PlayMode. Còn pixel/animation parity.
- `[~]` Đã port Settings core state, presenter và prefab: music/sound/vibration/people persistence, pattern mode/dot persistence, outgame/game-mode visibility, phản hồi toggle, source toast, GenericPopup timing, Restart/Terms/Privacy/Feedback và route Language/HTP có điều kiện. `SettingsLanguage/BlindMod/RuleText` dùng chung instance trong `AbConfigRuntime`, reload đúng `open_setting/game_start`. Pattern dùng đúng 12 sprite/màu nguồn; Language dropdown đóng ngay outside pointer-down, system option apply/persist rồi đóng; Restart có one-shot guard; Feedback/CMP/Terms/Privacy đi qua provider-neutral boundary dùng chung startup adapter và offline fallback không chặn UI. AppScene PlayMode đã bấm toàn bộ các đường này thật. Còn production SDK provider và pixel parity.
- `[~]` Đã copy nguyên CSV nguồn và port parser/catalog, locale alias/fallback/Chinese canonicalization, persistence, dynamic `LocalizedText`, font adapter zh/ja/ko, Language popup 10 hàng và System/English dropdown. Parser file thật, composition và show/hide/reopen đạt; dropdown non-English outside-close/system-confirm đã đạt AppScene PlayMode. Còn cold restart theo locale và device glyph.
- `[~]` Đã port cả `HowToPlay` toàn màn hình và `HowToPlayPaged`: matrix/coordinate nguồn, frame schedule, vòng demo, Previous/Next/Got it, localization/highlight, GenericPopup, silence/cleanup và cell cố định. AppScene đã bấm `Settings/HowToPlayBtn`, Previous/Next/Got it thật; `Closed` phát trước khi bắt đầu close animation đúng thứ tự Godot và lifecycle trở về Game sạch. Còn VFX/pixel parity và soak reopen dài.
- `[~]` Đã port Bank browser đủ sáu nhánh nguồn Regular/LK/LK Modified/LK Style/GC/SP: metadata, panel state, hard-tier split, selector, back stack, pooled row và launch params. AppScene PlayMode đã bấm UI động thật để launch/Win/Next đúng pool/index cho cả sáu nhánh; selector Tier/LK clamp đúng cả hai cận, launch đúng entry #2; Back đi đúng nhánh của cả sáu pool và 8 vòng Regular Root→Size→Tier→Back không tăng số row Size/Tier/Level. Direct-return chỉ hiện ở launch đầu, mất sau Next đúng nguồn; SP Fail/Restart giữ index/metadata. Còn profiler soak dài và pixel parity.
...
- `[x]` Daily/Streak/Rank/Profile entry dùng chung feature availability nguồn qua `AbConfigRuntime.Home`: offline default hiện Daily/Streak, ẩn Profile/Rank; provider có thể đổi đúng ba config. AppScene PlayMode xác nhận Daily khóa dưới level 21 không tạo page giả, Streak mở/Back thật, Daily level 21 mở `DailyGame` mode Daily, Rank ẩn dưới level 11 rồi mở popup/tham gia ở level 21 và Profile theo `leaderboard_func`.
...
- `[~]` Đã port Fail page, remaining cats, fail-text progress/promote, input lock, BGM/sound và restart. Nguồn Fail không có Home nên Unity không tự thêm nút này; lifecycle Main đã đạt PlayMode, còn animation/Spine và visual parity.
- `[~]` Đã port revive life/free-on-level/free-once, persisted `has_used_revive_free` và rewarded-ad boundary. Fail page dùng `AdService` với đúng position Main/Daily; null provider ẩn Revive đúng nguồn offline. Test provider chỉ thuộc PlayMode assembly đã xác nhận `ad_rewarded` hồi đúng 1 mạng/đóng Fail đúng một lần, còn `ad_closed` không cấp mạng và mở lại nút. Còn production SDK provider/device callback; không thêm mock-ad vào runtime.
- `[x]` Port level advance, clean win/fail/retry counters và DDA strategy update P0.
- `[x]` Port endgame snapshot clear/restore đúng thời điểm cho Win/Fail/Revive/Restart/Quit. Snapshot mang và khôi phục `in_game_sec`; focus-out/pause/quit force-rebuild trạng thái hiện tại như nguồn, đồng thời gộp cặp callback Unity thành một durability boundary.
- `[~]` Regression fixtures cho next bank, fail restart, elapsed clock, free revive, pass-text strategy, `StepsUsed`, special/hard advance và fail–revive nhiều lần đạt. AppScene PlayMode đã qua Main null-ad gate/Restart/Win/Next, rewarded revive success/close-without-reward, Bank sáu pool và lifecycle Playing→Fail→Revive→Win→Next. Hard-kill trên Android/iOS thật còn thuộc R17.
...
- `[~]` Đã port Daily entry state mở ở level 21, date/countdown/done format, persisted started/completed/max/elapsed/beat/best state, `DailyStats`, ba config `dc_level/no_dc/dc_tag_ui`, ngày neo 21/04/2026, selector pool/8 transform/invalid fallback và Home entry presenter ba trạng thái. AppScene PlayMode xác nhận entry khóa không mở page ở level 1, mở `DailyGame` với `GameplaySessionMode.Daily` ở level 21, Back về Home và đổi trạng thái Done→Normal khi resume qua ngày mới; visual parity còn chờ.
- `[~]` DailyGame đã chia sẻ `GameSession`, Board/input/tool/score/life feedback với game thường nhưng dùng `GameplaySessionMode.Daily` và coordinator riêng; Daily không đọc/ghi snapshot, retry, DDA, PreCat, Rank hay progress Main. AppScene Fail→rewarded Revive→Fail→Restart giữ đúng ngày/index/puzzle và toàn bộ sentinel Main; Back chỉ về Home như nguồn. Còn timer/ad compensation và device suspend/rollover thật.
- `[~]` Daily Win/Fail đã route riêng, revive/restart/win settlement idempotent, lưu elapsed/beat percent và không advance Main. Continue nay mở instance `Game` thật rồi đóng `DailyWin`/`DailyGame` đúng thứ tự nguồn, thay vì đổi mode ngay trong Daily page; time/beat/input gate 2 giây và toàn vòng đạt PlayMode. Còn tracker sink-capture, Spine adapter và pixel parity.
- `[x]` Đã port `ClockTicker` căn tick theo `ceil(now+0,001)`, local date key, không catch-up burst sau pause và day-watch cho Streak. Home nay dùng cùng clock scene-owned khi advance `max_daily_date`; AppScene PlayMode đã xác nhận focus/pause resume, rollover tiến/lùi ngày, Daily/Streak refresh và chống clock rollback. Device lifecycle thật được giữ ở R17.
- `[~]` Đã port Daily Streak core, persistence riêng, check-in/reward chu kỳ 7 ngày, page Main/Lit/Settle, Home entry, resume/backfill/protect, pending-win crash recovery, week slots, flow trước Result mặc định và settle-reorder sau Continue. AppScene đi đủ ngày 1→7→8, same-day idempotency và broken reset; slot mới chỉ sáng sau settle delay, chest ngày 7 cấp đúng +2 Hint/+2 Locate một lần. Restore đã nối rewarded callback `streak_revive_reward`; còn production provider và visual backfill/resume.
- `[~]` Đã port `AwardManager` với transaction in-flight ghi trước khi trình bày, cold-start sweep, nhận một lần, double chỉ cho tool, direct/streak/rank render boundary và Award page Collect offline. Rank Gift hai pha, podium/rương, `FrameAddEffect` và frame fly-to-profile đã nối ở R15; panel thường chỉ render tool, frame-only tự hoàn tất sau đúng nhịp nguồn. Rewarded-ad double/restore còn chờ R16.
...
- `[~]` Reflection regression Daily Streak/Award/Popup đạt 25/25, bao phủ đổi ngày, missed day, backfill/resume, pending-win crash, in-flight cold-start, double-complete, priority/DSL/switch occurrence và abort coroutine theo lifecycle Unity. Unity Test Runner, AppScene rollover và lịch Streak nhiều ngày đều đạt; còn provider/device kill và visual parity.
...
- `[~]` Đã port ProfileService/model/catalog, nickname 12 code point, 8 avatar + 8 classic frame + leaderboard frame 100, unlock/equip/count/red-dot, source-shaped remote export/merge và `profile.cfg` repository. Repository nay ghi đúng envelope Godot `profile/data`, vẫn đọc được flat schema của Unity cũ; file-backed restart giữ identity/frame inventory. `ProfileRuntime` scene-owned làm frame sink thật cho AwardManager; presenter/prefab/tabs/grid/lock-tip/pending Save/close đã có. Còn pixel/device parity.
- `[~]` Đã port RobotService/model/cache source-shaped: score generation, timeline/cooldown, identity, ranking tie-break, stalking/freeze/catch-up, effective-time clamp, `robots.cfg` repository và `RobotRuntime` scene-owned. Catalog 1.699 nickname giữ nguyên checksum nguồn; file-backed restart với clock rollback giữ nguyên pool/rank và period 2 giữ pool mới qua restart tiếp theo. Còn device/long-session soak.
- `[~]` Đã port `RankActivityManager`, ba group, period 24 giờ, điều kiện mở lại, level-cache chỉ commit khi thắng, điểm/rank/encouragement, expiry/settlement và reward table đúng nguồn. Hook start/win/restart/exit đã nối vào Gameplay theo đúng thứ tự trước transition và loại Daily khỏi lifecycle Rank. Manager regression xác nhận kỳ không thưởng giữ `NotOpened` qua 9 win và chỉ mở kỳ 2 ở win thứ 10; AppScene xác nhận expiry trong Main giữ settlement đến Win rồi chốt hạng 1 đúng một lần. File-backed cold-start nay xác nhận reward dở dang chỉ cấp một lần, pool cũ bị xóa và kỳ 2 mở/khôi phục đúng; còn device/long-session soak.
- `[~]` Đã port Home entry, open popup, leaderboard page, HTP và change page; prefab/registry dùng sprite nguồn, hierarchy theo nhánh chức năng và change timing/BBCode adapter theo Godot. Rank page nay giữ đúng VBox 1080×1920/2400, HeaderAdaptHolder/safe-area, list/CTA geometry, row spacing 20, scroll clamped và self-row nổi bám mép như `RankListViewImpl`. Row có `VisualRoot`, `Appear1/2/3` đúng key timing nguồn và self-shadow fade/lật theo mép trên/dưới. Rank Change dùng `Appear3`, safe groups, padding dọc 200, căn self-row giữa viewport, collection sáu Cat/Fish, arrow loop, lift/rise/drop, burst và domino/scroll-follow đúng timeline nguồn. AppScene xác nhận entry unlock, Action/Close, khóa dismiss trong animation và cleanup toàn flow. Còn pixel/video parity.
- `[~]` Rank reward đã nối `AwardManager`/Home popup/game win flow: pending reward, Rank Gift hai pha, top-3 podium, rương tier 3/2/1, frame-only, frame inventory và mở period kế tiếp. AppScene đi đủ cả group 1 Rank Change → podium/rương → item → Win và group 3 podium → `FrameAddEffect` tự đóng; frame chỉ persist sau effect, tool không đổi và transaction vẫn idempotent. Nhận trong Game không mở kỳ mới sớm, period 2 chỉ mở khi quay lại Home. Rank Gift nay giữ Appear1/Appear3, Open1 cue, celebration UGUI và frame fly-to-profile theo timing/curve nguồn; còn SFX, pixel/video parity và device soak.
...
- `[~]` Auth/device identity/API config: đã port `AuthService` provider-neutral từ `auth_manager.gd`, gate Analytics + LUID, init payload/guest login, 12 giây token timeout, relogin debounce 60 giây/tối đa 5, toàn bộ error code và `ApiConfig` endpoint/path/sign secret/platform đúng nguồn. `AuthRuntime` scene-owned đã nằm dưới `App/Systems`; khi thiếu native plugin hoạt động offline, không log và không chặn startup. Còn production iOS/Android provider cho AuthPlugin/UniKit device identity và device callback parity.
- `[~]` Data sync/merge conflict và startup timeout: đã port registry/late savable, bốn block `core/profile/streak/rank`, shared `remote_ahead`, unknown-field passthrough, snapshot baseline, meta fast-path, first upload, token refresh, conflict retry tối đa 3 và coalesced trigger. HTTP adapter giữ signing MD5, header, timeout 10 giây, server-Date calibration/retry; AppBootstrap chỉ chờ tối đa 2 giây và Home refresh khi remote thực sự đổi. File-backed snapshot/switch, EditMode/PlayMode đều đạt. Còn native UniKit metadata/Auth provider, backend thật và device offline/clock/app-kill parity.
- `[~]` Privacy/CMP/ATT/push permission/local notification: đã port `PrivacyPermissionRuntime` scene-owned và `IPlatformPermissionProvider` provider-neutral; giữ đúng privacy blocking → init ATT → startup push, Android CMP fire-and-return, iOS CMP timeout 2 giây → custom ATT/ATT system, ask-count/cooldown/cap, Win push-guide delay 2,467 giây, request type `System`/`SystemAndSetting`/`Setting`, tracker result và hai lịch noon/evening lặp vô hạn. Bốn popup Privacy/Pre-ATT/V2/Pre-Push dùng registry/prefab source-backed; AppScene đặt runtime dưới `App/Systems` và Settings dùng chung boundary. Offline provider không log/không chặn. Còn native iOS/Android adapter, OS dialog/device callback, notification persistence và pixel/device validation.
...
- `[~]` Lifecycle simulation đã khóa snapshot Playing/Fail/Revive/Win/Next, elapsed restore và callback dedup; save corruption, hard-kill thiết bị thật và update/migration vẫn còn.
...
| A-013 | Endgame `_request_save_endgame()` coalesce 0,5 giây | Unity adapter bắt buộc | Gameplay giữ đúng debounce/immediate của nguồn. Runtime repository chụp JSON bất biến, latest-wins coalesce và ghi mã hóa tuần tự trên worker. Focus-out/pause/quit force-rebuild snapshot với `in_game_sec`, dedup callback Unity rồi flush; constructor mặc định đồng bộ được giữ cho test/repository offline |
...
| A-029 | Godot có scene `DailyGame/DailyWin/DailyFail` riêng → Unity registry alias cùng prefab với nhánh session/presentation Daily | Unity adapter tổ chức UI | Mỗi `UiName` vẫn có instance/lifecycle/stack riêng; gameplay dùng chung `GameSession`, result presenter đổi text/time/beat và coordinator Daily cô lập state Main. Daily Continue mở `Game` rồi đóng `DailyWin/DailyGame` như nguồn, không tái dùng manager Daily làm Main; cây visual Daily chuyên biệt vẫn ở cùng prefab để không nhân bản logic |
...
| A-031 | Named-event bridge chạy Unity EditMode Test Runner | Editor-only automation | `Local\\Meowdoku.UnityEditModeTests` chạy full suite; hậu tố `.Platform` chỉ chạy fixture composition/platform liên quan và ghi result riêng dưới `Temp`. Cả hai gọi API Test Framework chính thức, không vào player, không log runtime và không thay gameplay |
| A-032 | Named-event bridge chạy Unity PlayMode Test Runner qua domain reload | Editor-only automation | `Local\\Meowdoku.UnityPlayModeTests` chạy full suite; hậu tố `.Platform` giữ filter ba flow platform qua `SessionState` và ghi result riêng. `IErrorCallbacks` cùng event `.Reset` phục hồi phiên bị Editor/player ngắt mà không để `RUNNING` kẹt. Test dùng scope state bộ nhớ tạm, giữ nguyên service/repository save thật |

| A-033 | Không tạo named-event bridge trong Unity `AssetImportWorker` | Editor-only process adapter | `AssetDatabase.IsAssetImportWorkerProcess()` chặn Refresh/EditMode/PlayMode bridge ở worker để `AutoResetEvent` chỉ được Editor chính tiêu thụ; loại bỏ việc lệnh tự động đôi lúc bị mất khi worker import preview |
| A-034 | Godot `CPUParticles2D` của Rank Change → pool Image + DOTween trên UGUI | Unity adapter bắt buộc | Giữ sáu vị trí collection, loại Cat/Fish theo group, stagger/arrival/duration, bốn arrow, glow/star burst và toàn bộ lift/rise/drop timing nguồn. Tween chạy unscaled, liên kết lifecycle row và reset khi hide/disable; không dùng runtime lookup hay log |
| A-035 | Godot Spine/`Line2D`/particle của Rank Gift và frame award → sprite nguồn + UGUI/DOTween | Unity adapter bắt buộc | Project không có Spine Unity runtime. Dùng chest tier sprite nguồn, CanvasGroup/RectTransform, trail 16 Image và burst Image cố định; giữ Appear1/Appear3, Open1 notify, cubic X/Y và profile shake đúng nguồn. Tween unscaled, cleanup theo page và không thêm runtime log |
| A-036 | Godot `Engine.get_singleton("AuthPlugin")` + signal JSON → `IAuthProvider`/`IAuthPrerequisiteProvider` | Unity adapter bắt buộc | Unity không có Godot Engine singleton/signal. Giữ nguyên JSON boundary, init/login/token/device API và policy trong `AuthService`; `AuthRuntime` sở hữu lifecycle, native adapter được tách riêng cho iOS/Android và null provider không chặn offline startup |
| A-037 | Godot static `DataSyncRegistry`/autoload coroutine → scene-owned registry/service/runtime | Unity adapter tổ chức mã | `DataSyncRuntime` sở hữu trigger/coalescing/cancellation và đăng ký bốn savable qua interface có kiểu; `DataSyncService` giữ nguyên merge/meta/upload/conflict policy. Snapshot dùng `SaveStore` verified file với schema logical `sync/remote_root`; không binary-compatible Godot ConfigFile |
| A-038 | Godot `HTTPRequest`/HttpClient → UnityWebRequest callback-coroutine adapter | Unity adapter bắt buộc | Giữ raw-body MD5 sign, timestamp/server-Date offset, headers, 10 giây timeout và retry sign/time một lần. Request được abort khi disable/destroy; UniKit metadata nằm sau interface và null metadata trả offline, không tự bịa SDK data |
| A-039 | Godot UniKit privacy/CMP/ATT/push API → `IPlatformPermissionProvider` + scene-owned runtime | Unity adapter bắt buộc | Unity không có Godot singleton/plugin signal. Runtime giữ ordering, save counter/date, A/B policy, popup callback và notification schedule; adapter native chỉ sở hữu OS/SDK call. `OfflinePlatformPermissionProvider` hoàn tất callback ngay, không đồng ý giả, không log và không chặn startup |
...
1. Tiếp tục R16 theo dependency: audit/port Feedback, Rate Us và Helpshift; sau đó gom production native Auth/UniKit/Data Sync/Privacy/ATT/Push adapter ở bước SDK/device integration.
2. Chạy hard-kill/resume và touch/notch trên Android/iOS thật; PlayMode/file-backed simulation đã đạt nhưng không thay thế device kill.
3. So pixel/timing 1080×1920/1080×2400 với reference nguồn và dùng Unity Profiler trên thiết bị ở R17.
...
- **2026-08-11:** bổ sung PlayMode Test Runner chịu domain reload và state scope không chạm save thật. AppScene đạt startup/8 route/cache/back, Button Home→Settings/Game→Home, Main result Fail→Restart→Win→Streak/Award→Next level 2, Bank SP card→Game→Return Bank và SP Win→Next #2→Fail→Restart. PlayMode lộ seed LK Style vượt Int32; đổi seed sang `long`, bỏ `from_bank_browser` ở Bank Next, refresh presentation khi tái sử dụng Game page và giữ pool/strategy metadata trong retry sau Next. Chặn AssetImportWorker khởi tạo ba named-event bridge để không nuốt lệnh tự động. Kết quả Unity thật: EditMode 510/510, PlayMode 5/5; không thêm runtime log.
- **2026-08-11:** khóa rewarded revive qua provider boundary bằng test provider chỉ compile trong PlayMode assembly, không chạm AppScene/runtime production. AppScene xác nhận đúng source position `normal_game_fail`: mở ad chưa hồi sinh, `ad_rewarded` hồi 1 mạng/đóng Fail đúng một lần, `ad_closed` sau reward không settle lặp; vòng Fail kế tiếp đóng ad không có reward vẫn giữ 0 mạng/Fail và mở lại Revive. Unity thật đạt EditMode 510/510, PlayMode 6/6; không thêm runtime log.
- **2026-08-11:** sửa thiếu sót A/B runtime ở R12: nguồn dùng config toàn cục và dye `open_setting`, trong khi Unity trước đó chỉ đăng ký nhóm Ads nên `settings_language/rule_text` production luôn rơi về default local. Thêm `SettingsConfigSet` vào `AbConfigRuntime`, cho AppBootstrap/Home/Settings/Game dùng chung instance và reload đúng timing. Test provider chỉ thuộc PlayMode xác nhận Button Home Settings→Language, Game Settings→HowToPlayPaged, visibility ngược nhau theo mode và lifecycle quay lại Game. Unity thật đạt EditMode 511/511, PlayMode 7/7; không thêm runtime log.
- **2026-08-11:** mở rộng Bank AppScene PlayMode từ SP sang toàn bộ Regular/LK/LK Modified/LK Style/GC. Test bấm root card, row Size/Tier/Level được materialize thật, launch entry #1, AutoComplete/Win/Next rồi xác nhận cùng pool và index kế tiếp; direct-return mất sau Next. Giữa các pool, Game được hide và Bank cùng instance render lại root; reopen Regular không tăng tổng `BankSizeCardView`. Kết quả Unity thật: PlayMode 8/8 trong 120,906 giây; EditMode gần nhất 511/511 và production assembly không đổi ngoài test accessor, không thêm runtime log.
- **2026-08-11:** đóng lát cắt transition/input guard R10 bằng đối chiếu trực tiếp `ui_manager.gd` cùng bốn call site Game/Fail/Daily Win/Win. Port local transparent blocker z=4095 có refresh deadline/cleanup mà không đổi tint button; nối terminal wrong guess 2,0 giây, Fail 1,5 giây và Daily Win 2,0 giây, giữ Win normal chỉ block khi rate/push branch tương lai thực sự chạy như nguồn. AppScene stress pointer release, 96 abort-close/reopen, mask/Z compact và concurrent one-flight; Fail flow xác nhận đồng thời blocker Game+Fail. Unity thật đạt EditMode 511/511 và PlayMode 9/9, không thêm runtime log.
- **2026-08-11:** khép interaction Settings pattern/HowToPlayPaged bằng đối chiếu `base_game_page.gd`, `board_view.gd`, `cell_view.gd`, `setting_page.gd` và `how_to_play_paged_page.gd`. Port đủ 12 pattern sprite/màu, hide/keep-on-filled, callback áp dụng tức thời, dismissed entry/switch state và signal `Closed` trước close animation. Installer idempotent ghi `Pattern` vào Cell prefab và palette vào GameplayScene/GamePage qua Unity serialization. AppScene bấm Sound/Vibration/People/Pattern, Previous/Next/Got it thật; Unity đạt EditMode 512/512 và PlayMode 9/9, không thêm runtime log.
- **2026-08-11:** khép selector/back-stack Bank R12 bằng đối chiếu trực tiếp `bank_page.gd`: Tier và LK selector bắt đầu ở 1, clamp `[1,count]`, GO dùng index đã chọn; Back giữ riêng Regular/LK Style/GC/LK/LK Modified/SP đúng nguồn. AppScene PlayMode bấm +/- đến cả hai cận, launch entry #2 cho cả selector Tier/LK, đi toàn bộ nhánh Back và stress 8 vòng Regular mà không tăng pool Size/Tier/Level. Thêm fallback phím tắt Editor-only cho Refresh/EditMode/PlayMode khi named event không tồn tại sau domain reload. Unity thật đạt EditMode 512/512 và PlayMode 9/9, không thêm runtime log.
- **2026-08-11:** khép interaction Settings còn lại bằng đối chiếu `language_switch_widget.gd`, `language_page.gd`, `setting_page.gd`, `privacy_dialog.gd` và `feedback_page.gd`. Đổi outside blocker từ Button release sang Graphic pointer-down, giữ option press/release target; system locale confirm apply/persist rồi đóng. Thêm `ISettingsExternalServices` dùng chung startup adapter cho online/CMP/FAQ/localized privacy URL với offline fallback; Restart giữ guard một lần. AppScene bấm offline/online Feedback, CMP, Terms/Privacy, outside-close, System option và double Restart thật. Unity đạt EditMode 512/512 và PlayMode 9/9, không thêm runtime log.
- **2026-08-11:** khép lifecycle snapshot Main bằng đối chiếu trực tiếp `game_page.gd`, `game_state.gd` và `session_manager.gd`. Sửa sai khác Unity còn thiếu `in_game_sec`: snapshot nay lưu/khôi phục elapsed time; lifecycle focus-out/pause/quit force-rebuild dù debounce đã idle, dedup cặp callback Unity và không tái tạo snapshot khi Won/Leaving. AppScene matrix chạy Playing MARK còn debounce → suspend → Fail → suspend → Revive → suspend → Win → suspend → Next level 2 → suspend; Unity thật đạt EditMode 513/513 và PlayMode 10/10, không thêm runtime log. Hard-kill thiết bị thật vẫn để R17.
- **2026-08-11:** khép feature availability Home R12 bằng đối chiếu `home_page.gd`, Daily/Streak/Rank entry và ba config nguồn. Sửa sai khác các consumer Unity tự giữ config local khiến remote Home flags không thể áp dụng thống nhất: thêm `HomeConfigSet` vào catalog `AbConfigRuntime`, inject cùng instance vào Home/Daily/Streak/Rank và giữ fallback nguồn. AppScene bấm entry thật qua locked Daily, Streak, Daily mode, Rank popup/Profile/Game; Unity đạt EditMode 514/514 và PlayMode 11/11, không thêm runtime log.
- **2026-08-11:** khép vòng Daily result R14 bằng đối chiếu `daily_game_page.gd`, `daily_fail_page.gd`, `daily_win_page.gd` và Rank hook chỉ có trong `game_page.gd`. Sửa hai sai khác Unity: shared manager từng gọi Rank lifecycle cho Daily, và Continue từng đổi manager Daily sang Main ngay trong sai page. Gate Rank khỏi Daily; Continue nay show `Game` rồi hide `DailyWin/DailyGame`. AppScene giữ sentinel Main snapshot/retry/DDA/strategy/stats và Rank cache qua Fail→rewarded Revive→Fail→Restart→Win, settle Daily rồi về Main level 21; Unity đạt EditMode 514/514 và PlayMode 12/12, không thêm runtime log.
- **2026-08-11:** xác nhận Rank close/reopen R15 bằng đối chiếu `home_page.gd`, `rank_activity_open_popup_page.gd`, `rank_activity_manager.gd` và `RankActivityPeriod`. AppScene bấm Close ở popup kỳ đầu: `WasStarted` vẫn false, popup đóng, manager vẫn confirm join rồi đi Profile guide/Game như nguồn. Manager integration kết thúc kỳ không thưởng, giữ đóng qua 9 Main win và chỉ mở kỳ 2 ở win thứ 10. Không cần sửa production; Unity đạt EditMode 515/515 và PlayMode 13/13, không thêm runtime log.
- **2026-08-11:** khép Daily rollover/focus R14 bằng đối chiếu `clock_ticker.gd`, `daily_entry_state.gd`, `home_page.gd` và `feature_daily_streak.gd`. Sửa Home dùng chung `ClockTicker.LocalNow` thay vì một nhánh `DateTime.Now` riêng khi advance `max_daily_date`. AppScene mô phỏng ngày 10→11 qua focus, reopen Home để persist ngày lớn nhất, rồi quay lùi qua pause; Daily entry, Streak signal và anti-clock-rollback đều đúng nguồn. Unity đạt EditMode 515/515 và PlayMode 14/14, không thêm runtime log.
- **2026-08-11:** khép lịch Streak nhiều ngày R14 bằng đối chiếu `feature_daily_streak.gd`, `streak_page.gd` và `streak_revive_flow.gd`. Sửa Settle từng bật sẵn ô check-in mới và dùng delay tăng số 0,9 giây để khóa Continue; nay ô mới ẩn đến đúng delay nguồn rồi reveal, Continue không bị khóa bởi effect tăng số. AppScene đi ngày 1→7→8, same-day no-op, bỏ ngày 18/reset ngày 19, thu durable chest đúng một lần; Unity đạt EditMode 515/515 và PlayMode 15/15, không thêm runtime log.
- **2026-08-11:** khép Rank expiry/reward/next-period R15 bằng đối chiếu `rank_activity_manager.gd`, `game_page.gd`, `home_page.gd`, Rank page/gift/open popup nguồn. Không phát hiện sai lệch production: expiry trong Main defer đến Win; sau collect flight chạy Rank Change, Rank Gift hai pha, fold về `NotOpened` mà chưa mở kỳ mới; khi Next rồi Back về Home mới mở period 2, Close xác nhận join nhưng không vào Game. Bổ sung fixture Rank/Robot/Profile/Award hoàn toàn trong bộ nhớ, không ghi save test; Unity compile sạch, EditMode 515/515 gần nhất và PlayMode 16/16, không thêm runtime log.
- **2026-08-11:** khép geometry/scroll/rise Rank UI R15 bằng đối chiếu trực tiếp `rank_activity_page.tscn/.gd`, `rank_activity_change_page.tscn/.gd`, `rank_list_view_impl.gd` và safe-area `ui_manager.gd`. Thay layout tạm của Unity bằng contract VBox đúng 1080×1920/2400, HeaderAdaptHolder, safe top/bottom, list 1008, CTA 784×258, row 968×180 cách 20; thêm self-row nổi, Change padding 200, center-scroll và rise/domino có scroll-follow. Prefab hiện hữu được nâng cấp qua Unity Prefab API, giữ GUID/reference; không sửa YAML bằng suy đoán. Unity compile sạch, EditMode 525/525 và PlayMode 16/16, không thêm runtime log.
- **2026-08-11:** khép Rank frame-only Award R15 bằng đối chiếu `award_page.gd/.tscn` và reward config nguồn. Sửa adapter Unity từng bật panel quà thường và render frame như item: nay panel chỉ nhận tool, frame dùng `FrameAwardEffectView` riêng với avatar/count động, Appear 0,5667 giây, hold 0,6334/0,8 giây và Disappear 0,3333 giây; inventory chỉ persist sau effect. Prefab được nâng cấp bằng Unity Prefab API. AppScene group 3 xác nhận panel item ẩn, +1 frame đúng một lần, tool không đổi và transaction rỗng; Unity đạt EditMode 527/527 và PlayMode 17/17, không thêm runtime log.
- **2026-08-11:** khép Rank row intro/self-shadow R15 bằng đối chiếu `rank_cell.gd/.tscn`, Rank page/Change và `rank_list_view_impl.gd`. Nâng prefab thành `VisualRoot/{Shadow, CanvasGroup}` để không tween RectTransform do layout sở hữu; port `Appear1` 0,3667 giây, `Appear2` 0,4215 giây, `Appear3` 0,3 giây cùng lịch delay Page/Change, shadow 1.033×270 fade 0,15 giây và lật ở mép trên. Prefab nâng cấp qua Unity API giữ GUID; bridge PlayMode bổ sung error/reset recovery sau một lượt player bị dừng. Unity cuối đạt EditMode 529/529 và PlayMode 17/17, không thêm runtime log.
- **2026-08-11:** khép collection/arrow/rise celebration của Rank Change R15 bằng đối chiếu `rank_change_collection.tscn`, `rank_cell_particles.tscn`, `rank_cell.gd` và `rank_activity_change_page.gd`. Port sáu Cat/Fish bay về target với stagger 0,1/arrival 1,0333/duration 1,6667 giây, bốn arrow loop, glow/edge-star burst, lift 0,2333 và drop 0,3333 giây; sửa nhánh không thăng hạng từng thiếu lift/drop và giữ swap final row tại 0,23 giây trong drop. Prefab được nâng cấp qua Unity API theo `VisualRoot/Effects/{Collection, Arrow, RiseBurst}`, tween cleanup khi hide/disable. Unity đạt EditMode 529/529 và PlayMode 17/17 trong 245,873 giây, không thêm runtime log.
- **2026-08-11:** khép Rank Gift chest/celebration và frame fly-to-profile R15 bằng đối chiếu `rank_gift_cell.gd/.tscn`, `award_page.gd`, `effect_score_trail.tscn`, `effect_score_burst.tscn` và `avatar_cell.tscn`. Sửa gate podium cũ 1,6 giây bằng Appear1 3,45/Appear3 3,3667 giây; Open1 giữ notify phase item tại 0,8834 và tổng 2 giây. Do project không có Spine Unity runtime/Line2D, adapter dùng sprite nguồn + UGUI/DOTween cho rương, bốn celebration burst, trail 16 đoạn, arrival burst và profile shake; đường bay giải cubic Bézier X/Y đúng nguồn. Prefab nâng cấp idempotent qua Unity API, tween cleanup theo lifecycle. Unity đạt EditMode 530/530 và PlayMode 17/17 trong 257,562 giây, không thêm runtime log.
- **2026-08-11:** khép cổng durability file-backed R15 bằng đối chiếu persistence Profile/Robot/Rank/Award nguồn. Sửa Profile Unity từng ghi flat section thành đúng envelope `profile/data`, đồng thời giữ reader tương thích file Unity cũ. Regression dùng `SaveStore`/repository thật qua nhiều lần tái tạo service: clock lùi không tua ngược robot rank, Rank reward dở dang được cold sweep đúng một lần, pool kỳ cũ bị xóa, Home mở kỳ 2 và pool mới sống qua restart tiếp theo. Unity đạt EditMode 532/532 trong 116,077 giây và PlayMode 17/17 trong 259,325 giây, không thêm runtime log.
- **2026-08-11:** bắt đầu Auth/API R16 bằng đối chiếu trực tiếp `auth_manager.gd` và `api_config.gd`. Port provider-neutral gate Analytics + LUID, payload init, guest login, token callback/timeout 12 giây, relogin 60 giây/tối đa 5, full error catalog, device/profile query và endpoint/path/sign secret/platform. `AuthRuntime` được installer đặt dưới `App/Systems`; null provider giữ offline startup im lặng. Đồng thời sửa `UnityRefreshBridge` bị kẹt pending nếu nhận tín hiệu trong lúc compile bằng polling retry theo Editor lifecycle. Unity đạt EditMode 537/537 trong 112,575 giây và PlayMode 17/17 trong 250,814 giây; native mobile provider/Data Sync còn ở phần tiếp theo.
- **2026-08-11:** port Data Sync R16 bằng đối chiếu toàn bộ manager/config/registry/API/HttpClient, bốn savable và launcher nguồn. Thêm scene-owned registry/service/runtime, `core/profile/streak/rank` merge, remote-ahead, unknown passthrough, file snapshot, meta fast-path, first upload, token refresh, ba conflict retry, coalescing và trigger level/profile/streak/late savable; Home chỉ refresh khi `changed=true`. UnityWebRequest adapter giữ sign/header/server clock/timeout 10 giây; AppBootstrap chờ tối đa 2 giây. Sửa cả EditMode/PlayMode bridge để pending request tự retry mỗi Editor update thay vì kẹt `delayCall`. Unity đạt EditMode 547/547 trong 125,636 giây và PlayMode 17/17 trong 260,772 giây; không có exception/log runtime mới.
- **2026-08-12:** port Privacy/CMP/ATT/push/local notification R16 bằng đối chiếu `launcher.gd`, `att_guide_helper.gd`, UniKit wrapper, bốn page nguồn, Win push-guide và A/B config. Thêm state/counter/cooldown, scene-owned provider-neutral runtime, Privacy/ATT/Pre-Push prefab/registry, Win delay 2,467 giây, tracker result và noon/evening local notification pool. Sửa generated MonoBehaviour phải nằm trong file trùng tên để Unity serialize MonoScript GUID; migration rebuild hai prefab có duplicate `closeButton`. Targeted Platform EditMode đạt 17/17 và PlayMode đạt 3/3; 558 test ngoài phạm vi đã pass ở lượt full trước lỗi composition và không bị chạy lặp sau sửa. Native mobile SDK/device validation còn mở.
